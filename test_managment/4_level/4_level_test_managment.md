...
Once testing activities are planned and executed, it is necessary to define a higher-level approach that guides how testing should be performed across the entire project. A test plan provides structure for a specific effort, but it does not fully define the long-term direction of testing.

Test strategy provides this direction.

Test strategy is a high-level approach that defines how testing will be carried out to achieve quality objectives. It ensures that testing activities are aligned with business goals, project risks and technical constraints.

Unlike a test plan, which is often specific to a release or phase, the test strategy is broader and more stable. It defines consistent principles and approaches that guide testing decisions throughout the project lifecycle.
...
Test strategy also defines the **testing techniques** that will be used. These may include **black-box testing**, which focuses on external behavior, and **white-box testing**, which uses knowledge of internal implementation. More advanced techniques are explored in **Test Case Design Level 4**.
